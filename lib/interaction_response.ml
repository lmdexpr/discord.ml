...
let string_of_t t = yojson_of_t t |> Yojson.Safe.to_string

let ok t = 
  let headers = [ "content-type", "application/json"] in
  headers, string_of_t t

let follow_up ~application_id ~discord_token ~interaction:Interaction.{ token; _ } msg =
  ...
  Client.post_request ~discord_token ~body uri
